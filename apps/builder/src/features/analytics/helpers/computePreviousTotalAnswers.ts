...
export const computePreviousTotalAnswers = (
  ...
  let totalAnswers = 0
  const allBlocks = publishedProbot.groups.flatMap((group) => group.blocks)
  const currentGroup = publishedProbot.groups.find((group) =>
  ...
  for (const block of previousBlocks.reverse()) {
    ...
    const connectedEdges = publishedProbot.edges.filter(
  ...
  const edgesConnectedToGroup = publishedProbot.edges.filter(
