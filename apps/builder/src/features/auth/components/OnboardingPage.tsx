...
export const OnboardingPage = () => {
  ...
  useEffect(() => {
    ...
    if (isNewUser === false || !env.NEXT_PUBLIC_ONBOARDING_PROBOT_ID)
      replace('/probots')
  ...
  return (
    <VStack h="100vh" flexDir="column" justifyContent="center" spacing="10">
      ...
      <Flex w="full" maxW="800px" h="full" maxH="70vh" rounded="lg">
        <Standard
          probot={env.NEXT_PUBLIC_ONBOARDING_PROBOT_ID}
          style={{ borderRadius: '1rem' }}
          prefilledVariables={{ Name: user?.name, Email: user?.email }}
          onEnd={() => {
            setTimeout(() => {
              push('/probots/create', { query: { isFirstBot: true } })
            }, 2000)
          }}
