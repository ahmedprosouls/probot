...
test.describe.parallel('Picture choice input block', () => {
  test('can edit items', async ({ page }) => {
    ...
    await expect(
      page.locator('probot-standard').getByText('Third image')
    ).toBeVisible()
    ...
    await expect(
      page.locator('probot-standard').getByText('First image, Second image')
    ).toBeVisible()
    ...
    await page.getByRole('button', { name: 'Go' }).click()
    await expect(
      page.locator('probot-standard').getByText('Second image')
    ).toBeVisible()
  })
})
