...
test.describe('Builder', () => {
  test('easy configuration should work', async ({ page }) => {
    ...
    await importProbotInDatabase(
      getTestAsset('probots/integrations/easyConfigWebhook.json'),
      {
        id: probotId,
      }
    )
    await createWebhook(probotId, { method: HttpMethod.POST })
  ...
  test('its configuration should work', async ({ page }) => {
    const probotId = createId()
    await importProbotInDatabase(
      getTestAsset('probots/integrations/webhook.json'),
      {
        id: probotId,
      }
    )
    ...
    await page.fill('input[placeholder="e.g. Content-Type"]', 'Custom-Probot')
    await page.fill(
...
test.describe('API', () => {
  const probotId = 'webhook-flow'

  test.beforeAll(async () => {
    try {
      await importProbotInDatabase(getTestAsset('probots/api.json'), {
        id: probotId,
      })
      await createWebhook(probotId)
    } catch (err) {
  ...
  test('can get webhook blocks', async ({ request }) => {
    const response = await request.get(
      `/api/v1/probots/${probotId}/webhookBlocks`,
      {
  ...
  test('can subscribe webhook', async ({ request }) => {
    ...
    const response = await request.post(
      `/api/v1/probots/${probotId}/webhookBlocks/webhookBlock/subscribe`,
      {
        headers: {
  ...
  test('can unsubscribe webhook', async ({ request }) => {
    const response = await request.post(
      `/api/v1/probots/${probotId}/webhookBlocks/webhookBlock/unsubscribe`,
      {
        headers: { Authorization: `Bearer ${apiToken}` },
      }
    )
  ...
  test('can get a sample result', async ({ request }) => {
    const response = await request.get(
      `/api/v1/probots/${probotId}/webhookBlocks/webhookBlock/getResultExample`,
      {
        headers: { Authorization: `Bearer ${apiToken}` },
      }
    )
