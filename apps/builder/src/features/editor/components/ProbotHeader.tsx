...
import { EditableProbotName } from './EditableProbotName'
...
export const ProbotHeader = () => {
  ...
  const handleNameSubmit = (name: string) =>
    updateProbot({ updates: { name } })

  const handleChangeIcon = (icon: string) =>
    updateProbot({ updates: { icon } })
  ...
  const handleHelpClick = () => {
    isCloudProdInstance()
      ? onOpen()
      : window.open('https://docs.probot.io', '_blank')
  }
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Button
          as={Link}
          href={`/probots/${probot?.id}/settings`}
      ...
      >
        <HStack alignItems="center" spacing={3}>
          <IconButton
            ...
            href={{
              pathname: router.query.parentId
                ? '/probots/[probotId]/edit'
                : probot?.folderId
                ? '/probots/folders/[folderId]'
                : '/probots',
              query: {
                ...
                parentId: Array.isArray(router.query.parentId)
                  ...
                  : [],
                probotId: Array.isArray(router.query.parentId)
          ...
          <HStack spacing={1}>
            ...
            (
            <EditableProbotName
              key={`probot-name-${probot?.name ?? ''}`}
              defaultName={probot?.name ?? ''}
      ...
      <HStack right="40px" pos="absolute" display={['none', 'flex']}>
        <CollaborationMenuButton isLoading={isNotDefined(probot)} />
        {router.pathname.includes('/edit') && isNotDefined(rightPanel) && (
          <Button
            colorScheme="gray"
            onClick={handlePreviewClick}
            isLoading={isNotDefined(probot)}
            size="sm"
          >
