...
const itemsAction = (setProbot: SetProbot): ItemsActions => ({
  createItem: (
    item: NewItem,
    ...
    setProbot((probot) =>
      produce(probot, (probot) => {
        const block = probot.groups[groupIndex].blocks[
          blockIndex
  ...
  updateItem: (
    ...
    setProbot((probot) =>
      produce(probot, (probot) => {
        ...
        ;(
          probot.groups[groupIndex].blocks[blockIndex] as BlockWithItems
        ).items[itemIndex] = {
  ...
  deleteItem: ({ groupIndex, blockIndex, itemIndex }: ItemIndices) =>
    setProbot((probot) =>
      produce(probot, (probot) => {
        ...
        block.items.splice(itemIndex, 1)
        cleanUpEdgeDraft(probot, removingItem.id)
      })
    ),
})
