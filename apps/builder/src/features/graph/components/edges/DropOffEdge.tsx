...
export const DropOffEdge = ({
  ...
  const { totalDroppedUser, dropOffRate } = useMemo(() => {
    if (!publishedProbot || currentBlock?.total === undefined)
  ...
  const sourceTop = useMemo(() => {
    ...
    const block = publishedProbot?.groups
  ...
  }, [currentBlock?.blockId, publishedProbot?.groups, sourceEndpoints])

  const endpointCoordinates = useMemo(() => {
    const groupId = publishedProbot?.groups.find((group) =>
  ...
  const isLastBlock = useMemo(() => {
    if (!publishedProbot) return false
    const lastBlock = publishedProbot.groups
      .find((group) =>
        group.blocks.some((block) => block.id === currentBlock?.blockId)
      )
      ?.blocks.at(-1)
    return lastBlock?.id === currentBlock?.blockId
  }, [publishedProbot, currentBlock?.blockId])
