...
export const ItemNode = ({
  ...
  const isConnectable =
    ...
    !connectionDisabled &&
    !(
      probot.groups[indices.groupIndex].blocks[indices.blockIndex] as
        | ChoiceInputBlock
        | undefined
        ...
        >
          ...
          >
            <ItemNodeContent
              ...
              indices={indices}
            />
            {probot && (isConnectable || pathname.endsWith('analytics')) && (
              <SourceEndpoint
                source={{
                  groupId: probot.groups[indices.groupIndex].id,
                  blockId:
                    probot.groups[indices.groupIndex]?.blocks[
                      indices.blockIndex
                    ]?.id,
                  itemId: item.id,
                }}
