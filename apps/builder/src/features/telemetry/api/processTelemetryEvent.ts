...
// Only used for the cloud version of Probot. It's the way it processes telemetry events and inject it to thrid-party services.
export const processTelemetryEvent = authenticatedProcedure
  ...
  .mutation(async ({ input: { events }, ctx: { user } }) => {
    ...
    events.forEach(async (event) => {
      ...
      if (
        event.name === 'Probot created' ||
        event.name === 'Probot published'
      )
        client.groupIdentify({
          groupType: 'probot',
          groupKey: event.probotId,
      ...
      const groups: { workspace?: string; probot?: string } = {}
      if ('workspaceId' in event) groups['workspace'] = event.workspaceId
      if ('probotId' in event) groups['probot'] = event.probotId
      client.capture({
        distinctId: event.userId,
        event: event.name,
        properties: event.data,
