...
test.describe.parallel('Theme page', () => {
  test.describe('General', () => {
    test('should reflect change in real-time', async ({ page }) => {
      ...
      await importProbotInDatabase(getTestAsset('probots/theme.json'), {
        id: probotId,
      })
      await page.goto(`/probots/${probotId}/theme`)
      await expect(page.getByRole('button', { name: 'Go' })).toBeVisible()

      // Branding
      await page.getByRole('button', { name: 'Global' }).click()
      await expect(
        page.locator('a:has-text("Made with Probot")')
      ).toHaveAttribute('href', 'https://www.probot.io/?utm_source=litebadge')
      await page.click('text="Show Probot brand"')
      await expect(page.locator('a:has-text("Made with Probot")')).toBeHidden()

      // Font
      await page.getByRole('textbox').fill('Roboto Slab')
      await expect(page.locator('.probot-container')).toHaveCSS(
        'font-family',
        /"Roboto Slab"/
      )
  ...
  test.describe('Chat', () => {
    test('should reflect change in real-time', async ({ page }) => {
      const probotId = 'chat-theme-probot'
      try {
        await importProbotInDatabase(getTestAsset('probots/theme.json'), {
          id: probotId,
        })
      ...
      await expect(page.locator('.probot-container img')).toHaveAttribute(
        'src',
        hostAvatarUrl
      )
      await page.click('text=Bot avatar')

      await expect(page.locator('.probot-container img')).toBeHidden()
      ...
      const input = page.locator('.probot-input')
  ...
  test.describe('Custom CSS', () => {
    test('should reflect change in real-time', async ({ page }) => {
      ...
      await page.fill(
        'div[role="textbox"]',
        '.probot-button {background-color: green}'
      )
      await expect(page.getByRole('button', { name: 'Go' })).toHaveCSS(
        'background-color',
        'rgb(0, 128, 0)'
  ...
  test.describe('Theme templates', () => {
    test('should reflect change in real-time', async ({ page }) => {
      ...
      await page.getByText('Probot Dark').click()
...
test.describe('Free workspace', () => {
  test("can't remove branding", async ({ page }) => {
    const probotId = createId()
    await importProbotInDatabase(getTestAsset('probots/settings.json'), {
      id: probotId,
