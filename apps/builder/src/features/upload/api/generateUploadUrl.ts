...
const parseFilePath = async ({
  ...
  if (
    !probot ||
    (await isWriteProbotForbidden(probot, {
  ...
  if (!('blockId' in input)) {
    return `public/workspaces/${input.workspaceId}/probots/${input.probotId}/${input.fileName}`
  }
  return `public/workspaces/${input.workspaceId}/probots/${
    input.probotId
