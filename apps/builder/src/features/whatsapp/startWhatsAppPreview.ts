...
import { isReadProbotForbidden } from '../probot/helpers/isReadProbotForbidden'
...
export const startWhatsAppPreview = authenticatedProcedure
  .meta({
    openapi: {
      method: 'POST',
      path: '/probots/{probotId}/whatsapp/start-preview',
  ...
  .mutation(
    async ({ input: { to, probotId, startGroupId }, ctx: { user } }) => {
      if (
