...
const proProbotId = createId()
const starterProbotId = createId()
...
test('can switch between workspaces and access probot', async ({ page }) => {
  await page.goto('/probots')
  await expect(page.locator('text="Pro probot"')).toBeVisible({
    timeout: 20000,
  })
  await page.click('text=Pro workspace')
  await page.click('text="Starter workspace"')
  await expect(page.locator('text="Pro probot"')).toBeHidden()
  await page.click('text="Starter probot"')
...
test('can update workspace info', async ({ page }) => {
  ...
  await page.getByTestId('probot-logo').click({ force: true })
  await expect(
