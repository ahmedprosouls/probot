      ...
      switch (event.type) {
        ...
        case 'customer.subscription.deleted': {
          ...
          const probots = await prisma.probot.findMany({
            ...
            include: { publishedProbot: true },
          })
          for (const probot of probots) {
            const settings = probot.settings as Settings
            ...
            const publishedProbotSettings = probot.publishedProbot
              ?.settings as Settings | null
            if (
              !publishedProbotSettings ||
              publishedProbotSettings?.general.isBrandingEnabled
            )
              continue
            await prisma.publicProbot.updateMany({
              ...
              data: {
                settings: {
                  ...publishedProbotSettings,
                  general: {
                    ...publishedProbotSettings.general,
                    isBrandingEnabled: true,
                  },
                },
              },
