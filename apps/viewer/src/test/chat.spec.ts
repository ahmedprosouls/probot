...
test.afterEach(async () => {
  ...
  await deleteprobots(['chat-sub-bot'])
})

test('API chat execution should work on preview bot', async ({ request }) => {
  const probotId = createId()
  const publicId = `${probotId}-public`
  await importprobotInDatabase(getTestAsset('probots/chat/main.json'), {
    id: probotId,
    publicId,
  })
  await importprobotInDatabase(getTestAsset('probots/chat/linkedBot.json'), {
    id: 'chat-sub-bot',
...
test('API chat execution should work on published bot', async ({ request }) => {
  ...
  await test.step('Answer Email question with valid input', async () => {
    const { messages, input } = await (
      await request.post(`/api/v1/sendMessage`, {
        data: { message: 'probot@email.com', sessionId: chatSessionId },
      })
    ).json()
  ...
  await test.step('Answer URL question', async () => {
    const { messages, input } = await (
      await request.post(`/api/v1/sendMessage`, {
        data: { message: 'https://probot.io', sessionId: chatSessionId },
      })
    ).json()
    expect(messages.length).toBe(0)
