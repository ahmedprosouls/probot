...
test('should work as expected', async ({ page, browser }) => {
  ...
  await importprobotInDatabase(getTestAsset('probots/fileUpload.json'), {
    id: probotId,
  ...
  await page
    ...
    .setInputFiles([
      getTestAsset('probots/api.json'),
      getTestAsset('probots/fileUpload.json'),
      getTestAsset('probots/hugeGroup.json'),
    ])
