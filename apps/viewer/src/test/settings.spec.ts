...
test('Hide query params', async ({ page }) => {
  ...
  await page.goto(`/${probotId}-public?Name=John`)
  await page.waitForTimeout(1000)
  expect(page.url()).toEqual(`http://localhost:3001/${probotId}-public`)
  await updateprobot({
    id: probotId,
    settings: {
      ...settings,
  ...
  expect(page.url()).toEqual(
    `http://localhost:3001/${probotId}-public?Name=John`
  )
})
