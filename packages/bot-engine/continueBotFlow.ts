...
export const continueBotFlow =
  ...
  async (
    ...
    const group = state.probotsQueue[0].probot.groups.find(
    ...
    if (block.type === LogicBlockType.SET_VARIABLE) {
      const existingVariable = state.probotsQueue[0].probot.variables.find(
    ...
    if (isInputBlock(block)) {
      ...
      const itemId = nextEdgeId
        ? newSessionState.probotsQueue[0].probot.edges.find(byId(nextEdgeId))
            ?.from.itemId
        : undefined
    ...
    if (!nextEdgeId && state.probotsQueue.length === 1)
      return {
        messages: [],
        newSessionState,
        lastMessageNewFormat:
...
const saveVariableValueIfAny =
  ...
  (reply: string): SessionState => {
    ...
    const foundVariable = state.probotsQueue[0].probot.variables.find(
...
const saveAnswer =
  ...
  async (reply: string): Promise<SessionState> => {
    ...
    const key = block.options.variableId
      ? state.probotsQueue[0].probot.variables.find(
          (variable) => variable.id === block.options.variableId
        )?.name
      : state.probotsQueue[0].probot.groups.find((group) =>
...
const setNewAnswerInState =
  (state: SessionState) => (newAnswer: AnswerInSessionState) => {
    const answers = state.probotsQueue[0].answers
...
const getOutgoingEdgeId =
  (state: Pick<SessionState, 'probotsQueue'>) =>
  (block: Block, reply: string | undefined) => {
    const variables = state.probotsQueue[0].probot.variables
    if (
