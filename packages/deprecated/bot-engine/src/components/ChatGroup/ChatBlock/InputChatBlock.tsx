...
export const InputChatBlock = ({
  ...
  const { probot, isLoading } = useProbot()
  ...
  const defaultValue =
    (probot.settings.general.isInputPrefillEnabled ?? true) && variableId
      ? probot.variables.find(
          (variable) =>
            variable.name === probot.variables.find(byId(variableId))?.name
        )?.value
      : undefined
  ...
  const handleSubmit = async ({ label, value, itemId }: InputSubmitContent) => {
    ...
    if (!isRetry && addAnswer)
      await addAnswer(probot.variables)({
        blockId: block.id,
  ...
  if (answer) {
    const avatarUrl = probot.theme.chat.guestAvatar?.url
    return (
      <GuestBubble
        message={answer}
        showAvatar={probot.theme.chat.guestAvatar?.isEnabled ?? false}
        avatarSrc={avatarUrl && parseVariables(probot.variables)(avatarUrl)}
      />
    )
  }
