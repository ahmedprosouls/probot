...
export const ConversationContainer = ({
  ...
  const displayNextGroup = ({
    ...
    const currentProbot = updatedProbot ?? probot
    if (groupId) {
      const nextGroup = currentProbot.groups.find(byId(groupId))
      if (!nextGroup) return
      onNewGroupVisible({
        id: 'edgeId',
    ...
    const nextEdge = currentProbot.edges.find(byId(edgeId))
    if (!nextEdge) {
    ...
    const nextGroup = currentProbot.groups.find(byId(nextEdge.to.groupId))
  ...
  useEffect(() => {
    ...
    setHasStarted(true)
    const startEdge = probot.groups[0].blocks[0].outgoingEdgeId
  ...
  return (
    <div
      ...
      className="overflow-y-scroll w-full lg:w-3/4 min-h-full rounded lg:px-5 px-3 pt-10 relative scrollable-container probot-chat-view"
    >
