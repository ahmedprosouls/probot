...
export const EmbedBubble = ({ block, onTransitionEnd }: Props) => {
  ...
  const [url] = useState(parseVariables(probot.variables)(block.content?.url))
  ...
  const height = block.content.height
    ? typeof block.content.height === 'string'
      ? parseVariables(probot.variables)(block.content.height) + 'px'
  ...
  return (
    <div className="flex flex-col w-full" ref={messageContainer}>
      <div className="flex mb-2 w-full lg:w-11/12 items-center">
        <div
          className={
            'flex relative z-10 items-start probot-host-bubble w-full'
          }
        >
          <div
