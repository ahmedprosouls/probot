...
export const FileUploadForm = ({
  ...
  const { isPreview, currentProbotId } = useProbot()
  ...
  const startSingleFileUpload = async (file: File) => {
    ...
    const urls = await uploadFiles({
      basePath: `/api/probots/${currentProbotId}/blocks/${id}`,
      files: [
        {
          file,
  ...
  return (
    <form className="flex flex-col w-full" onSubmit={handleSubmit}>
      <label
        ...
        className={
          'probot-upload-input py-6 flex flex-col justify-center items-center w-full bg-gray-50 rounded-lg border-2 border-gray-300 border-dashed cursor-pointer hover:bg-gray-100 px-8 mb-2 ' +
      ...
      {selectedFiles.length === 0 && isRequired === false && (
        <div className="flex justify-end">
          <button
            className={
              'py-2 px-4 justify-center font-semibold rounded-md text-white focus:outline-none flex items-center disabled:opacity-50 disabled:cursor-not-allowed disabled:brightness-100 transition-all filter hover:brightness-90 active:brightness-75 probot-button '
            }
            onClick={onSkip}
          >
