...
export const initBubble = (params: BubbleParams): BubbleActions => {
  ...
  const existingBubble = document.getElementById('probot-bubble') as
    | HTMLDivElement
    | undefined
...
const createBubble = (
  ...
  bubbleElement.id = 'probot-bubble'
...
export const getBubbleActions = (
  ...
  const existingBubbleElement =
    bubbleElement ??
    (document.querySelector('#probot-bubble') as HTMLDivElement | undefined)
  if (!existingBubbleElement) return { close: () => {}, open: () => {} }
  const existingProactiveMessage =
    proactiveMessageElement ??
    document.querySelector('#probot-bubble .proactive-message')
  return {
