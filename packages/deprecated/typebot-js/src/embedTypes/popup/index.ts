...
export const initPopup = (params: PopupParams): PopupActions => {
  ...
  const existingPopup = document.getElementById('probot-popup')
...
const createOverlayElement = (delay: number | undefined) => {
  ...
  overlayElement.id = 'probot-popup'
...
export const getPopupActions = (
  ...
  const existingPopupElement =
    popupElement ?? (document.querySelector('#probot-popup') as HTMLDivElement)
  return {
    open: () => {
