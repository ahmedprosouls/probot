...
describe('initPopup', () => {
  ...
  it('should return the popupElement with lazy iframe', () => {
    expect.assertions(2)
    initPopup({ url: 'https://probot.io/probot-id' })
    const popupElement = document.getElementById('probot-popup')
  ...
  it('should overwrite if exists', () => {
    ...
    initPopup({ url: 'https://probot.io/probot-id2' })
    const elements = document.getElementsByTagName('iframe')
    expect(elements).toHaveLength(1)
    expect(elements[0].dataset.src).toBe('https://probot.io/probot-id2')
  })
  ...
  it('should have the opened classname after the delay', async () => {
    expect.assertions(2)
    initPopup({ delay: 500, url: 'https://probot.io/probot-id' })
    const popupElement = document.getElementById('probot-popup')
    expect(popupElement?.classList.contains('opened')).toBe(false)
...
describe('openPopup', () => {
  ...
  it('should add opened className and lazy load', () => {
    expect.assertions(5)
    const { open } = initPopup({ url: 'https://probot.io/probot-id' })
    const popupElement = document.getElementById('probot-popup')
...
describe('closePopup', () => {
  ...
  it('shouldn remove opened className', () => {
    expect.assertions(2)
    const { close } = initPopup({ url: 'https://probot.io/probot-id' })
    const popupElement = document.getElementById('probot-popup')
    close()
    expect(popupElement?.classList.contains('opened')).toBe(false)
...
describe('Request commands afterwards', () => {
  it('should return defined commands', () => {
    ...
    open()
    const popup = document.getElementById('probot-popup') as HTMLDivElement
