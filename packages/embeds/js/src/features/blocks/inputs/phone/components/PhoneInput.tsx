...
export const PhoneInput = (props: PhoneInputProps) => {
  ...
  const processIncomingEvent = (event: MessageEvent<CommandData>) => {
    const { data } = event
    if (!data.isFromProbot) return
    if (data.command === 'setInputValue') setInputValue(data.value)
  }

  return (
    <div
      class={'flex items-end justify-between pr-2 probot-input'}
      data-testid="input"
      style={{
        'max-width': '400px',
      }}
    ...
    >
      <div class="flex">
        <div class="relative probot-country-select flex justify-center items-center">
