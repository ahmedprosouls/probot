...
export const parseResultHeader = (
  probot: Pick<Probot, 'groups' | 'variables'>,
  linkedProbots: Pick<Probot, 'groups' | 'variables'>[] | undefined,
  results?: ResultWithAnswers[]
): ResultHeaderCell[] => {
  const parsedGroups = [
    ...probot.groups,
    ...(linkedProbots ?? []).flatMap((linkedProbot) => linkedProbot.groups),
  ]
  const parsedVariables = [
    ...probot.variables,
    ...(linkedProbots ?? []).flatMap(
      (linkedProbot) => linkedProbot.variables
    ),
  ]
